...
def find_similar_shots(nhl_csv, my_csv, row_index, feature_weights, top_n):
    ...
    xGs = nhl['xGoal'].iloc[top_indices]
...
vals= []
sum = 0
for i in range(208,len(df1)):
    j = 0
    chance = find_similar_shots(df, df1, i, feature_weights=feature_weights, top_n=10)
